...
test('schema changeTable add foreign key', async () => {
  ...
  `));
    expect(trim(sql.next())).toBe(trim('CREATE INDEX "tablesOtherTableIdIndex" ON "tables" ("otherTableId")'));
    await reverse.changeTable(table, tableCallback);
    expect(trim(sql.next())).toBe(trim(`
    ALTER TABLE "tables"
...
test('schema change methods', async () => {
  ...
  `));
    ...
    expect(sql.next()).toBe(`SELECT 1 FROM "information_schema"."columns" WHERE "table_name" = 'table' AND "column_name" = 'column'`);
